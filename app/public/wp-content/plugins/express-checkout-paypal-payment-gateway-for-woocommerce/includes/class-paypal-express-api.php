...
class Eh_PayPal_Express_Payment extends WC_Payment_Gateway {
			...
			"
                        jQuery( function( $ ) {
                           ...
                           var eh_paypal_disable_funding    = jQuery( '#woocommerce_eh_paypal_express_disable_funding_source').closest( 'tr' );
							...
							$( '#woocommerce_eh_paypal_express_smart_button_size' ).on( 'change', function( event ) {

                                var isResponsive = 'responsive' !== $( event.target ).val();
                                var table      = $( event.target ).closest( 'table' );
                                var tagline  = table.find( '#woocommerce_eh_paypal_express_button_tagline');
                                var tagline_option = tagline.find( 'option[value=\"show\"]' );
                                if ( tagline_option.prop( 'disabled' ) !== isResponsive ) {
                                    tagline.removeClass( 'enhanced' );
                                    tagline_option.prop( 'disabled', isResponsive );
                                    $( document.body ).trigger( 'wc-enhanced-select-init' );
                                    ! tagline.val() && tagline.val( 'hide' ).trigger( 'change' );
                                }
                           ...
                           var express_elements   = jQuery( '.express_toggle_display').closest( 'table' );
                            ...
                            $('input.eh_paypal_mode').on('change', function() { 
                                $('input.eh_paypal_mode').prop('checked', true); 
		...
		switch ( $action_code ) {
			...
			case 'credit_start':
				try {
					...
					/**
                    * Alter the cancel URL.
                    *
                    * @since 1.8.6
                    *
                    * @param string $cancel_url Cancel URL.
                    * @param string $page_referer Page referer.
                    *
                    * @return  string Cancel URL. 
                    */
                    $cancel_url = apply_filters('eh_paypal_order_cancel_url', $cancel_url, $page_referer);
			...
			case 'express_details':
				...
				try {
					...
					if ( ! empty( $is_express ) && 'true' === $is_express ) {
						...
						$alter_billing = apply_filters('eh_paypal_alter_billing_address_with_paypal_response', true);
	...
	public function add_cancel_order_elements() {
		if ( isset( WC()->session->eh_pe_checkout ) ) {
			...
			$page = get_permalink();
			if ($this->get_option('smart_button_enabled') == 'yes') {
                printf('<a href="' . esc_url(add_query_arg('p', $page, eh_paypal_express_run()->hook_include->make_express_url('cancel_order'))) . '" class="button alt" style="background-color: crimson; text-align: center; margin-top: %s;">' . esc_html__('Cancel Order', 'express-checkout-paypal-payment-gateway-for-woocommerce') . '</a>', '5%');

            }else{
                printf('<a href="' . esc_url(add_query_arg('p', $page, eh_paypal_express_run()->hook_include->make_express_url('cancel_express'))) . '" class="button alt" style="background-color: crimson; text-align: center; margin-top: %s;">' . esc_html__('Cancel Order', 'express-checkout-paypal-payment-gateway-for-woocommerce') . '</a>', '5%');
